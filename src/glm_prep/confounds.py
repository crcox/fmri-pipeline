...
from glm_prep.models import ACompCorMask
...
_TEDANA_REQUIRED_METRICS: tuple[str, ...] = (
    "kappa",
    "rho"
)
...
def parse_confounds_timeseries(df: pd.DataFrame) -> tuple[MotionConfounds, ACompCorConfounds, DriftConfounds]:
    motion: dict[str, Vector] = {col: get_vector(df, col) for col in df.columns if col.startswith(("trans_", "rot_"))}
    acompcor: dict[str, Vector] = {col: get_vector(df, col) for col in df.columns if col.startswith("a_comp_cor")}
    cosine: dict[str, Vector] = {col: get_vector(df, col) for col in df.columns if re.fullmatch(r"cosine_?\d{2}", col)}
...
def parse_tedana_metrics(df: pd.DataFrame) -> TedanaMetadata:
    validate_tedana_metrics(df, _TEDANA_REQUIRED_CLASSIFICATION + _TEDANA_REQUIRED_METRICS)
    ...
    for row in records:
        ...
        raw_tags = cast(str, row["classification_tags"])
        tags = {t.strip() for t in raw_tags.split(",") if t}
...
def parse_acompcor_metadata(json_dict: dict[str, Any]) -> ACompCorMetadata:
    ...
    for name, data in json_dict.items():
        ...
        if not isinstance(data, dict):
            raise DataContractError(
                f"Metadata entry for {name!r} must be an object"
            )

        try:
            mask = ACompCorMask(data['Mask'])
            retained = bool(data['Retained'])
            singular_value = float(data['SingluarValue'])
